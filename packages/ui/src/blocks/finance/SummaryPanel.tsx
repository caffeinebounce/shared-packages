    ...
    <ResponsiveContainer width="100%" height={height}>
      ...
      >
        ...
        <YAxis
          ...
          tickLine={false}
          axisLine={false}
          tickFormatter={(v) => formatFn(v as number)}
          width={65}
        />
        {showZeroLine && (
          <ReferenceLine
        ...
        <Tooltip
          content={<CustomTooltip series={series} formatFn={tooltipFormat} />}
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -10 }}>
        ...
        <Tooltip
          content={<CustomTooltip series={series} formatFn={tooltipFormat} />}
          cursor={{ fill: "var(--muted, #f3f4f6)", fillOpacity: 0.3 }}
        />
        {series.map((s) => (
          <Bar
            key={s.key}
...
export {
  AreaLineChart as SummaryAreaChart,
  ColumnChart as SummaryBarChart,
  PieChartView as SummaryPieChart,
};
...
export function SummaryPanel({
  ...
  const [activeChartType, setActiveChartType] =
    React.useState<SummaryChartType>(defaultChartType);

  const formatFn = formatValue ?? defaultFormat;
  const hasChart =
    (!!chartData && chartData.length > 0 && !!chartSeries) || !!renderChart;
    ...
    <div className={cn("rounded-lg border bg-card", className)}>
      ...
      {expanded && (
        <div className="px-4 pb-4">
          ...
          >
            {metrics.map((m) => {
              const isPositiveTrend = m.invertTrend
                ? (m.trend ?? 0) < 0
                : (m.trend ?? 0) > 0;
              return (
                <div
          ...
          {hasChart && (hasChartToggle || chartControls) && (
            <div className="mb-3 flex items-center justify-between">
              ...
              {chartControls && (
                <div className="flex items-center">{chartControls}</div>
              )}
            </div>
          )}
          ...
          {hasChart && (
            <div
              style={{
                width: "100%",
                ...(renderChart ? {} : { height: chartHeight }),
              }}
            >
              {renderChart ? (
                renderChart(
